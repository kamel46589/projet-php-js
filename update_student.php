...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $updated_student_card_number = $student_card_number;
    $updated_email = $email;

    // Check if fields are not empty, if they are, keep the existing value
    ...
    if (!empty($_POST['student_card_number'])) {
        $updated_student_card_number = $_POST['student_card_number'];
    }
    ...
    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param("ssssi", $updated_first_name, $updated_last_name, $updated_student_card_number, $updated_email, $student_id);
